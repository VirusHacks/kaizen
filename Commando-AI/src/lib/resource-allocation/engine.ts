...
export function calculateDeliveryConfidence(state: PlanningState): number {
  ...
  const unassignedHighPri = activeTasks.filter(
    (t) =>
      !t.assigneeId && (t.priority === 'HIGH' || t.priority === 'CRITICAL'),
  );
  ...
  if (
    ...
    const estimatedRemainingHours = sprintActive.reduce(
      (sum, t) => sum + t.estimatedHours,
      0,
    );
    const availableTeamHours =
      team.reduce((sum, m) => sum + m.availableHours, 0) * avgVelocity;
    const workFitRatio =
      availableTeamHours > 0 ? estimatedRemainingHours / availableTeamHours : 2;
  ...
  if (team.length >= 2) {
    ...
    const variance =
      utilizations.reduce((sum, u) => sum + (u - avgUtil) ** 2, 0) /
      utilizations.length;
    ...
    if (task.assigneeId) {
      ...
      if (assignee) {
        if (assignee.utilizationPercent > 140) {
          riskScore += 30;
          reasons.push(
            `Assignee ${assignee.userName} is severely overloaded (${Math.round(assignee.utilizationPercent)}%)`,
          );
        } else if (assignee.utilizationPercent > 120) {
          riskScore += 20;
          reasons.push(
            `Assignee ${assignee.userName} is overloaded (${Math.round(assignee.utilizationPercent)}%)`,
          );
        } else if (assignee.utilizationPercent > 100) {
          riskScore += 12;
          reasons.push(
            `Assignee ${assignee.userName} is over capacity (${Math.round(assignee.utilizationPercent)}%)`,
          );
        }
        ...
        if (assignee.burnoutRisk > 80) {
          riskScore += 18;
          reasons.push(
            `Assignee ${assignee.userName} has critical burnout risk (${Math.round(assignee.burnoutRisk)}%)`,
          );
        } else if (assignee.burnoutRisk > 60) {
          riskScore += 10;
          reasons.push(
            `Assignee ${assignee.userName} has high burnout risk (${Math.round(assignee.burnoutRisk)}%)`,
          );
        }
      }
    }
...
export function generateRecommendations(
  ...
  recommendations.push(
    ...generateReassignmentRecommendations(state, config, outcomes),
  );
  recommendations.push(
    ...generateDelayRecommendations(state, config, outcomes),
  );
  recommendations.push(
    ...generateRebalanceRecommendations(state, config, outcomes),
  );
  recommendations.push(...generateReviewerRecommendations(state, outcomes));
  recommendations.push(
    ...generateUnassignedRecommendations(state, config, outcomes),
  );
  ...
  for (const rec of recommendations) {
    ...
    rec.impact.overallScore =
      rec.impact.overallScore * 0.8 +
      explorationBonus * rec.impact.overallScore * 0.2;
  }
...
function generateReassignmentRecommendations(
  ...
  const overloaded = state.team.filter(
    (m) =>
      m.utilizationPercent > 100 || m.burnoutRisk > config.burnoutThreshold,
  );
  ...
  for (const overMember of overloaded) {
    const memberTasks = state.tasks
      .filter((t) => t.assigneeId === overMember.userId && t.status !== 'DONE')
      .sort((a, b) => {
        const priOrder = { LOW: 0, MEDIUM: 1, HIGH: 2, CRITICAL: 3 };
        return (
          (priOrder[a.priority as keyof typeof priOrder] ?? 1) -
          (priOrder[b.priority as keyof typeof priOrder] ?? 1)
        );
      });
    ...
    for (const task of memberTasks.slice(0, 3)) {
      ...
      const deliveryChange = calculateDeliveryImpact(
        task,
        overMember,
        bestTarget,
        skillMatch,
      );
      const costChange = calculateCostImpact(
        overMember,
        bestTarget,
        task.estimatedHours,
      );
...
function generateDelayRecommendations(
  ...
  for (const task of state.tasks) {
    ...
    if ((isBlocked && isAtRisk) || (isOverdue && !isBlocked)) {
      const suggestedDelay = isOverdue
        ? Math.max(3, Math.abs(daysUntilDue) + 2)
        : 3;
      ...
      recs.push({
        ...
        action: {
          ...
          reason: isBlocked
            ? `Blocked by ${blockedDeps.length} dependencies`
...
function generateRebalanceRecommendations(
  ...
  if (spread > 40) {
    const mostLoaded = state.team.find(
      (m) => m.utilizationPercent === maxUtil,
    )!;
    const leastLoaded = state.team.find(
      (m) => m.utilizationPercent === minUtil,
    )!;
  ...
  for (const task of inReviewTasks) {
    ...
    const scoredReviewers = availableReviewers
      .map((m) => ({
        member: m,
        score:
          calculateSkillMatch(task, m) * 50 +
          ((100 - m.utilizationPercent) / 100) * 50,
      }))
...
function generateUnassignedRecommendations(
  ...
  const available = state.team
    .filter(
      (m) =>
        m.utilizationPercent < 85 && m.burnoutRisk < config.burnoutThreshold,
    )
  ...
  for (const task of unassigned) {
    ...
    const scoredMembers = available
      .map((m) => ({
        member: m,
        skillMatch: calculateSkillMatch(task, m),
        score:
          calculateSkillMatch(task, m) * 40 +
          ((100 - m.utilizationPercent) / 100) * 40 +
          m.velocity * 20,
      }))
      .sort((a, b) => b.score - a.score);
    ...
    const overallScore =
      deliveryImpact * config.deliverySlippageWeight + best.skillMatch * 5;
...
function calculateBurnoutImpact(from: CapacityInfo, to: CapacityInfo): number {
  const fromReduction =
    from.burnoutRisk > 70 ? -Math.min(20, from.burnoutRisk * 0.25) : -5;
