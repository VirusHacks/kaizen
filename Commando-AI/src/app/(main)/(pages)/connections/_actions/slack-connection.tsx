...
const ensureUserInDb = async (clerkId: string) => {
  ...
  console.log('Auto-created/updated user in database for Slack connection:', clerkId)
  return true
}
...
export const onSlackConnect = async (
  ...
  const userExists = await ensureUserInDb(user_id)
  if (!userExists) {
    console.error('Failed to ensure user exists for Slack connection')
    return
  }
