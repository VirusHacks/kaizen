...
export default function OrdersForecast({ periods }: { periods: number }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/forecast?type=orders&periods=${periods}`)
      .then(async (res) => {
        const data = await res.json();
        
        if (!res.ok || data.error) {
          console.error("[OrdersForecast] Error:", data.error || `HTTP ${res.status}`);
      ...
      .catch((err) => {
        console.error("[OrdersForecast] Fetch error:", err);
  ...
  if (loading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Orders Forecast (Prophet Model)</CardTitle>
