...
import { Target, TrendingDown, Filter } from 'lucide-react';
...
export default function LeadConversionFunnel({
  ...
  const contactedPct =
    totalLeads > 0 ? ((contactedLeads / totalLeads) * 100).toFixed(1) : '0';
  const qualifiedPct =
  ...
  const contactDropOff =
    totalLeads > 0 ? ((1 - contactedLeads / totalLeads) * 100).toFixed(1) : '0';
  const qualifyDropOff =
    contactedLeads > 0
      ? ((1 - qualifiedLeads / contactedLeads) * 100).toFixed(1)
      : '0';
  const convertDropOff =
    qualifiedLeads > 0
      ? ((1 - convertedLeads / qualifiedLeads) * 100).toFixed(1)
      : '0';
      ...
      <Card className="bg-[#0a0a0a] border border-gray-800 overflow-hidden">
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 pt-4 border-t border-gray-800">
            {[
              {
                label: 'Contact Drop-off',
                value: contactDropOff,
                color: 'text-red-400',
              },
              {
                label: 'Qualify Drop-off',
                value: qualifyDropOff,
                color: 'text-orange-400',
              },
              {
                label: 'Convert Drop-off',
                value: convertDropOff,
