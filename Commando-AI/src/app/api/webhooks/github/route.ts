...
async function handlePush(body: any) {
  ...
  // Find projects linked to this repo and could trigger automations
  try {
    ...
    for (const setup of linkedSetups) {
      console.log(
        `[GITHUB_WEBHOOK] Push relates to project: ${setup.project.name} (${setup.project.key})`
      )
      // Future: trigger workflow automations, update activity feed, etc.
    }
  } catch (err) {
