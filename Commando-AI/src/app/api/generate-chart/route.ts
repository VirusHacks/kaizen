...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      '[Generate Chart API] Available data sources:',
    ...
    if (!apiKey) {
      ...
      return NextResponse.json(
        {
          error:
            'Google AI API key not configured. Set GEMINI_API_KEY or GOOGLE_AI_API_KEY in .env',
        },
        { status: 500 },
      );
    }
    ...
    if (!chartConfig) {
      return NextResponse.json(
        {
          error: 'Failed to parse chart configuration from AI response',
          rawResponse: text,
        },
        { status: 500 },
      );
    ...
    if (availableData && availableData[dataSourceType]) {
      ...
      const similarKey = keys.find((k) =>
        k.toLowerCase().includes(dataSourceType.toLowerCase()),
      );
    ...
    if (
      ...
      errorDetails =
        'Please add GOOGLE_AI_API_KEY to your environment variables.';
