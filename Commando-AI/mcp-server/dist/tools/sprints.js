...
export function registerSprintTools(server) {
    // ─── Get all sprints ───
    server.tool('get_sprints', 'List all sprints in a project with their status, dates, and issue counts.', { projectId: z.string().describe('Project ID') }, async ({ projectId }) => {
    ...
    server.tool('get_active_sprint', 'Get the currently active sprint with its tasks and progress. Shows you what needs to be done before the sprint ends.', { projectId: z.string().describe('Project ID') }, async ({ projectId }) => {
    ...
    server.tool('get_backlog', 'Get all issues not assigned to any sprint (the backlog). Useful for finding unplanned work.', { projectId: z.string().describe('Project ID') }, async ({ projectId }) => {
