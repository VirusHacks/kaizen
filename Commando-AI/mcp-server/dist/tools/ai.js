...
export function registerAITools(server) {
    ...
    server.tool('generate_implementation_plan', 'Generate a step-by-step implementation plan for a task using AI. Returns architecture steps, file changes, database setup, integration steps, and testing plan. Use this before starting a complex task.', {
...
${issue.parent ? `**Parent ${issue.parent.type}:** ${issue.parent.title}\n${issue.parent.description ? `**Parent Description:** ${issue.parent.description}` : ''}` : ''}`;
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                if (start !== -1 && end !== -1) {
                    ...
                    return { content: [{ type: 'text', text: `AI response could not be parsed:\n${text}` }], isError: true };
                }
            }
        }
        return {
    ...
    server.tool('ask_project_question', 'Ask a question about the project, its architecture, or a specific task. The AI uses project context, tech stack, and coding standards to answer. Great for "how should I implement X?" questions.', {
...
Answer concisely and specifically for this project's tech stack. Give code examples when helpful.`;
    ...
    server.tool('generate_test_cases', 'Generate test cases for a task based on its description and acceptance criteria. Returns test scenarios with expected behavior.', { issueId: z.string().describe('Issue/task ID') }, async ({ issueId }) => {
        const issue = await db.issue.findUnique({
            where: { id: issueId },
            include: {
                parent: { select: { title: true, description: true } },
...
Include positive cases, negative cases, and edge cases. No markdown fences, just JSON.`;
    ...
    server.tool('check_acceptance_criteria', 'Given a task, generate a checklist of acceptance criteria and review points to verify before marking the task as done.', { issueId: z.string().describe('Issue/task ID') }, async ({ issueId }) => {
        const issue = await db.issue.findUnique({
            where: { id: issueId },
            include: {
                parent: { select: { title: true, description: true } },
