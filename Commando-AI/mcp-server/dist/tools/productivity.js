...
export function registerProductivityTools(server) {
    // ─── Get developer stats (personal dashboard) ───
    server.tool('get_my_stats', 'Get your personal stats for a project: tasks by status, priority breakdown, completion rate, and active sprint info.', {
        projectId: z.string().describe('Project ID'),
        userId: z.string().describe('Clerk user ID'),
    }, async ({ projectId, userId }) => {
        const [total, todo, inProgress, inReview, done, critical, high, bugs, overdue] = await Promise.all([
            ...
            db.issue.count({ where: { projectId, assigneeId: userId, isArchived: false, type: 'BUG' } }),
            db.issue.count({
                where: {
                    projectId, assigneeId: userId, isArchived: false,
                    status: { not: 'DONE' },
                    dueDate: { lt: new Date() },
                },
            }),
        ...
        const activeSprint = await db.sprint.findFirst({
            ...
            select: {
                ...
                _count: { select: { issues: { where: { assigneeId: userId, isArchived: false } } } },
                issues: {
        ...
        const sprintInfo = activeSprint ? {
            ...
            myDone: activeSprint.issues.filter((i) => i.status === 'DONE').length,
        } : null;
        const result = {
            total,
    ...
    server.tool('generate_standup', 'Generate a daily standup summary: what was done recently, what you are working on, and blockers. Based on your assigned tasks and recent status changes.', {
        projectId: z.string().describe('Project ID'),
        userId: z.string().describe('Clerk user ID'),
    }, async ({ projectId, userId }) => {
        const now = new Date();
        ...
        const recentlyDone = await db.issue.findMany({
            ...
            select: {
                number: true, title: true, type: true,
        ...
        const inProgress = await db.issue.findMany({
            ...
            select: {
                number: true, title: true, type: true, priority: true,
                project: { select: { key: true } },
            },
        });
        // In review
        ...
        const overdue = await db.issue.findMany({
            ...
            select: {
                number: true, title: true, dueDate: true,
                project: { select: { key: true } },
            },
        });
        // Critical items
        const critical = await db.issue.findMany({
            where: {
                projectId, assigneeId: userId, isArchived: false,
                status: { not: 'DONE' }, priority: 'CRITICAL',
            },
            select: {
        ...
        const fmt = (i) => `${i.project.key}-${i.number}: ${i.title}`;
        const standup = {
            date: now.toISOString().split('T')[0],
            yesterday: recentlyDone.length > 0
            ...
            blockers: [
                ...overdue.map((i) => `OVERDUE: ${fmt(i)} (due ${i.dueDate?.toISOString().split('T')[0]})`),
                ...critical.map((i) => `CRITICAL: ${fmt(i)}`),
            ],
        };
        // Format as readable text
        let text = `## Daily Standup — ${standup.date}\n\n`;
        text += `### ✅ Yesterday (Done)\n${standup.yesterday.map((t) => `- ${t}`).join('\n')}\n\n`;
        text += `### 🔨 Today (In Progress)\n${standup.today.map((t) => `- ${t}`).join('\n')}\n\n`;
        if (standup.inReview.length > 0) {
            text += `### 👀 In Review\n${standup.inReview.map((t) => `- ${t}`).join('\n')}\n\n`;
        }
        if (standup.blockers.length > 0) {
            text += `### 🚨 Blockers / Risks\n${standup.blockers.map((t) => `- ${t}`).join('\n')}\n`;
        }
        else {
            text += `### 🚨 Blockers\n- None\n`;
        }
    ...
    server.tool('get_branch_name', 'Generate a git branch name for a task following conventions. E.g., feature/PROJ-123-add-user-auth. Use when creating a branch to work on a task.', { issueId: z.string().describe('Issue/task ID') }, async ({ issueId }) => {
        ...
        const prefix = issue.type === 'BUG' ? 'bugfix'
    ...
    server.tool('get_commit_message', 'Generate a conventional commit message for a task. Includes the task key in the format: type(scope): description [PROJ-123]', {
        issueId: z.string().describe('Issue/task ID'),
        changeDescription: z.string().describe('Brief description of what changed'),
        commitType: z.enum(['feat', 'fix', 'refactor', 'docs', 'test', 'chore', 'style', 'perf']).default('feat').describe('Conventional commit type'),
    ...
    server.tool('get_next_task', 'Get the next most important task to work on. Prioritizes by: overdue > critical > high priority > sprint tasks > others. Helps decide what to pick up next.', {
        projectId: z.string().describe('Project ID'),
        userId: z.string().describe('Clerk user ID'),
    }, async ({ projectId, userId }) => {
        ...
        const overdue = await db.issue.findFirst({
            ...
            include: { project: { select: { key: true } }, sprint: { select: { name: true } } },
            orderBy: { dueDate: 'asc' },
        });
        if (overdue) {
            return {
        ...
        return {
            content: [{ type: 'text', text: '🎉 All tasks are done or in review! No pending TODO tasks.' }],
        };
    });
}
