...
export function registerProjectTools(server) {
    ...
    server.tool('list_projects', 'List all projects the user is a member of or owns. Returns project ID, name, key, and description.', { userId: z.string().describe('Clerk user ID of the developer') }, async ({ userId }) => {
    ...
    server.tool('get_project_context', 'Get full project context including tech stack, vision, AI instructions, team members, GitHub repo info, and counts. Essential for understanding what a project is about before working on tasks.', { projectId: z.string().describe('Project ID') }, async ({ projectId }) => {
    ...
    server.tool('get_coding_standards', 'Get the project coding standards, AI instructions, and tech stack. Use this to understand how code should be written for this project.', { projectId: z.string().describe('Project ID') }, async ({ projectId }) => {
        const project = await db.project.findUnique({
            where: { id: projectId },
            select: {
                name: true, key: true,
    ...
    // ─── Get project members ───
    server.tool('get_team_members', 'List all team members in a project with their roles and department roles.', { projectId: z.string().describe('Project ID') }, async ({ projectId }) => {
