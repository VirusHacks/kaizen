...
export function registerTaskTools(server: McpServer) {
  ...
  server.tool(
    'get_my_tasks',
    'Get all tasks assigned to you in a project. Returns task key, title, type, status, priority, sprint, and description. Use this to see what you need to work on.',
    {
  ...
  server.tool(
    'get_task_details',
    'Get complete details of a specific task including description, subtasks, parent story, sprint info, and project tech stack. Use this before starting work on a task.',
  ...
  server.tool(
    'find_task_by_key',
    'Find a task by its project key and number (e.g., "PROJ-42"). Use when you know the task key but not the ID.',
    {
    ...
    async ({ projectKey, taskNumber }) => {
      ...
      if (!issue) {
        return {
          content: [
            {
              type: 'text' as const,
              text: `Task ${projectKey}-${taskNumber} not found`,
            },
          ],
          isError: true,
        };
      ...
      const result = {
        ...
        parent: issue.parent
          ? `${projectKey}-${issue.parent.number}: ${issue.parent.title}`
          : null,
  ...
  server.tool(
    'update_task_status',
    'Update the status of a task (e.g., move from TODO to IN_PROGRESS when you start working, or to DONE when finished). This changes the kanban board position.',
    {
      issueId: z.string().describe('Issue/task ID'),
      status: z
        .enum(['TODO', 'IN_PROGRESS', 'IN_REVIEW', 'DONE'])
  ...
  server.tool(
    ...
    'Update or append to a task description. Useful for documenting findings, implementation notes, or context discovered during development.',
    {
      issueId: z.string().describe('Issue/task ID'),
      ...
      append: z
        ...
        .default(false)
        .describe(
          'If true, appends to existing description instead of replacing',
        ),
    },
  ...
  server.tool(
    'search_tasks',
    'Search for tasks in a project by title or description keywords. Useful to find related tasks or check if a task already exists.',
    {
      projectId: z.string().describe('Project ID'),
      query: z
        .string()
        .describe('Search query (searches in title and description)'),
      type: z
  ...
  server.tool(
    'create_task',
    'Create a new task/issue in a project. Use when you discover something that needs to be done, like a bug or a subtask.',
    {
      projectId: z.string().describe('Project ID'),
      ...
      description: z
        .string()
        .optional()
        .describe('Task description with context and acceptance criteria'),
      type: z
        .enum(['EPIC', 'STORY', 'TASK', 'BUG', 'SUBTASK'])
        .default('TASK')
        .describe('Issue type'),
      ...
      assigneeId: z
        .string()
        .optional()
        .describe('Clerk user ID to assign (defaults to unassigned)'),
      reporterId: z.string().describe('Clerk user ID of the reporter (you)'),
      parentId: z
        .string()
        .optional()
        .describe('Parent issue ID (for subtasks)'),
  ...
  server.tool(
    'get_all_tasks',
    'Get all tasks in a project grouped by status. Provides a high-level overview of project progress.',
    {
      projectId: z.string().describe('Project ID'),
      sprintId: z.string().optional().describe('Filter to a specific sprint'),
    },
    async ({ projectId, sprintId }) => {
      const where: Record<string, unknown> = { projectId, isArchived: false };
      ...
      const grouped = {
        TODO: issues
          .filter((i: { status: string }) => i.status === 'TODO')
          .map(fmt),
        IN_PROGRESS: issues
          .filter((i: { status: string }) => i.status === 'IN_PROGRESS')
          .map(fmt),
        IN_REVIEW: issues
          .filter((i: { status: string }) => i.status === 'IN_REVIEW')
          .map(fmt),
        DONE: issues
