...
export function registerTaskTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ projectKey, taskNumber }) => {
      ...
      if (!project) {
        return { content: [{ type: 'text' as const, text: `Project with key "${projectKey}" not found` }], isError: true }
      }
      ...
      if (!issue) {
        return { content: [{ type: 'text' as const, text: `Task ${projectKey}-${taskNumber} not found` }], isError: true }
      }
...
function fmt(i: { id: string; number: number; title: string; type: string; priority: string; assignee: { name: string | null } | null; project: { key: string } }) {
  return {
    id: i.id,
    key: `${i.project.key}-${i.number}`,
    title: i.title,
